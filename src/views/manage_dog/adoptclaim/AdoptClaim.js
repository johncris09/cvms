...
const AdoptClaim = ({ roleType, userId }) => {
  const _table = 'adopt'
        ...
        if (snapshot.exists()) {
          ...
          const processedData = filteredData.map((item) => {
            return {
              ...
              adopt_owner_name: item.adopt_owner_name,
              adopt_address: item.adopt_address,
  ...
  const handleExportRows = (rows) => {
    ...
    const exportedData = rows
      ...
      .map((item) => {
        return {
          date: item.adopt_date,
          owner_name: item.adopt_owner_name,
          address: item.adopt_address,
          status: item.status,
          pet_name: item.pet_name,
          pet_name: item.pet_name,
          color: item.color,
  ...
  const handleExportData = () => {
    ...
    const exportedData = data.map((item) => {
      ...
      columns.forEach((column) => {
        rowData[column.header] = item[column.accessorKey]
      })
      return rowData
    })
            ...
            <>
              {roleType !== 'User' && (
                <MaterialReactTable
                  ...
                  initialState={{
                    density: 'compact',
                    // columnPinning: { right: ['owner_name', 'or_number'] },
                  }}
